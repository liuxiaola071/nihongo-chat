...
const SERVER_URL = localStorage.getItem('nihongo_server') || (isApp ? 'http://10.240.124.21:8765' : '');
const API = SERVER_URL + '/api/chat';

// ==================== 全局网络错误检测 ====================
let _lastNetErrorTime = 0;

/** 包装 fetch：网络断开时给用户一个明确提示（防抖，3秒内只提示一次） */
...
async function tryUnlock() {
  ...
  errEl.textContent = '';
  try {
    const r = await fetch(SERVER_URL + '/api/unlock', {
      method: 'POST',
...
async function fetchFurigana(text) {
  ...
  const resp = await fetch(SERVER_URL + '/api/furigana', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    credentials: 'include',
...
async function speakSentence(text) {
  ...
  try {
    const resp = await fetch(SERVER_URL + '/api/tts', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
...
async function switchScenario(sid) {
  ...
  try {
    const resp = await fetch(SERVER_URL + `/api/scenario/${encodeURIComponent(sid)}`, { method: 'POST' });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
...
async function gradeWord(quality) {
  ...
  try {
    const resp = await fetch(SERVER_URL + '/api/srs/grade', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
...
async function loadQuiz(level) {
  try {
    ...
    const resp = await fetch(`${SERVER_URL}/api/assist/quiz?count=20${diffParam}`, {
      credentials: 'include',
    });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const data = await resp.json();
...
document.getElementById('export-btn')?.addEventListener('click', async () => {
  try {
    const resp = await fetch(SERVER_URL + '/api/export/markdown', { credentials: 'include' });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
...
async function loadStats() {
  try {
    const resp = await fetch(SERVER_URL + '/api/stats', { credentials: 'include' });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
...
async function loadErrors() {
  ...
  try {
    const resp = await fetch(SERVER_URL + '/api/errors', { credentials: 'include' });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const data = await resp.json();
...
document.getElementById('errors-clear')?.addEventListener('click', async () => {
  ...
  try {
    await fetch(SERVER_URL + '/api/errors', { method: 'DELETE', credentials: 'include' });
    loadErrors();
    showNotice('クリアしました', 1500);
  } catch (err) {
...
async function loadCounter() {
  try {
    const resp = await fetch(SERVER_URL + '/api/assist/counter?count=5', { credentials: 'include' });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const data = await resp.json();
